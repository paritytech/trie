...
pub use trie_db::{
    decode_compact, encode_compact, NibbleSlice, NibbleVec, NodeCodec, Recorder, Trie, TrieDB,
    TrieDBMut, TrieDBNodeIterator, TrieError, TrieIterator, TrieMut,
};
pub use trie_db::{nibble_ops, Record, TrieConfiguration, TrieLayout};
...
impl TrieStream for ReferenceTrieStreamNoExt {
    ...
    fn append_leaf(&mut self, key: &[u8], value: &[u8]) {
        self.buffer
            .extend(fuse_nibbles_node_no_extension(key, NodeKindNoExt::Leaf));
        value.encode_to(&mut self.buffer);
    }
...
mod tests {
    ...
    fn too_big_nibble_length() {
        ...
        let enc =
            <ReferenceNodeCodecNoExt<KeccakHasher> as NodeCodec>::leaf_node(((0, 0), &input), &[1]);
