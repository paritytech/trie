...
use reference_trie::{RefHasher, test_layouts, ExtensionLayoutHybrid,
	NoExtensionLayout, ExtensionLayout, NoExtensionLayoutHybrid};
...
fn compare_implementations_no_extension_unordered(data: Vec<(Vec<u8>, Vec<u8>)>) {
	...
	reference_trie::compare_implementations_unordered::<NoExtensionLayout, _>(data.clone(), memdb.clone(), hashdb.clone());
	reference_trie::compare_implementations_unordered::<NoExtensionLayoutHybrid, _>(data, memdb, hashdb);
}
...
fn too_big_nibble_length_old () {
	compare_implementations_prefixed_internal::<reference_trie::ExtensionLayout>(
...
fn too_big_nibble_length_new () {
	...
	compare_implementations_prefixed_internal::<reference_trie::NoExtensionLayout>(data.clone());
	compare_implementations_prefixed_internal::<reference_trie::NoExtensionLayoutHybrid>(data.clone());
}
#[test]
fn polka_re_test () {
	compare_implementations(vec![
