	...
	L: TrieLayout
{
	...
	pub fn new(db: &'db mut dyn HashDB<L::H, DBValue>, root: &'db mut TrieHash<L>) -> Self {
	...
	pub fn db(&self) -> &dyn HashDB<L::H, DBValue> { self.raw.db() }

	/// Get the backing database.
	pub fn db_mut(&mut self) -> &mut dyn HashDB<L::H, DBValue> { self.raw.db_mut() }
}
	...
	L: TrieLayout,
{
	...
	fn insert(&mut self, key: &[u8], value: &[u8]) -> Result<Option<DBValue>, TrieHash<L>, CError<L>> {
	 ...
	 fn remove(&mut self, key: &[u8]) -> Result<Option<DBValue>, TrieHash<L>, CError<L>> {
		self.raw.remove(&L::H::hash(key).as_ref())
	}
}
