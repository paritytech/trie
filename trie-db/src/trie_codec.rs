...
use crate::core_::{convert::TryInto, marker::PhantomData, result};
...
impl<'a, C: NodeCodec> DecoderStackEntry<'a, C> {
    ...
    fn encode_node(self) -> Vec<u8> {
        match self.node {
            ...
            Node::Leaf(partial, value) => C::leaf_node(partial.right(), value),
    ...
    DB: HashDB<L::Hash, T>,
{
    ...
    Err(Box::new(TrieError::IncompleteDatabase(
        <TrieHash<L>>::default(),
    )))
}
...
mod tests {
    ...
    use reference_trie::{
        decode_compact, encode_compact, ExtensionLayout, NoExtensionLayout, Recorder, Trie, TrieDB,
        TrieDBMut, TrieError, TrieLayout, TrieMut,
    };
