...
impl<'a> NibbleSlice<'a> {
    ...
    pub fn right(&'a self) -> Partial {
        ...
        if nb > 0 {
            (
                (nb, nibble_ops::pad_right(self.data[split])),
                &self.data[split + 1..],
            )
        } else {
            ((0, 0), &self.data[split..])
    ...
    pub fn left(&'a self) -> Prefix {
        ...
        if ix == 0 {
            ...
            (
                &self.data[..split],
                Some(nibble_ops::pad_left(self.data[split])),
            )
        }
    }
...
mod tests {
    ...
    fn encoded_pre() {
        ...
        assert_eq!(
            n.to_stored(),
            (0, BackingByteVec::from_slice(&[0x01, 0x23, 0x45]))
        );
        assert_eq!(
            n.mid(1).to_stored(),
            (1, BackingByteVec::from_slice(&[0x01, 0x23, 0x45]))
        );
        assert_eq!(
            n.mid(2).to_stored(),
            (0, BackingByteVec::from_slice(&[0x23, 0x45]))
        );
        assert_eq!(
            n.mid(3).to_stored(),
            (1, BackingByteVec::from_slice(&[0x23, 0x45]))
        );
    }
    ...
    fn range_iter() {
        ...
        for i in [vec![], vec![0x03], vec![0x34], vec![0x03, 0x45]]
            .iter()
            .enumerate()
        {
