...
use crate::node::{decode_hash, Node, NodeHandle, NodeHandleOwned, NodeOwned};
    ...
    Q: Query<L::Hash>,
{
    ...
    fn look_up_with_cache_internal(
        ...
        for depth in 0.. {
            ...
            self.recorder.record(TrieAccess::NodeOwned {
                hash,
                node_owned: node,
            });
    ...
    pub fn traverse_to(mut self, key: &[u8]) -> Result<(), TrieHash<L>, CError<L>> {
        match self.cache.take() {
            Some(cache) => self
                .look_up_with_cache(key, NibbleSlice::new(key), cache)
                .map(drop),
