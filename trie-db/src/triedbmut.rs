        ...
        let node = match encoded_node {
            ...
            EncodedNode::Branch(encoded_children, val) => {
                let mut child = |i: usize| match encoded_children[i] {
                    Some(child) => {
                        Self::inline_or_hash::<C, H>(node_hash, child, db, storage).map(Some)
                    }
                    None => Ok(None),
                };
...
mod tests {
    ...
    use reference_trie::{
        reference_trie_root, reference_trie_root_no_extension, NodeCodec, RefTrieDBMut,
        RefTrieDBMutNoExt, ReferenceNodeCodec, TrieMut,
    };
