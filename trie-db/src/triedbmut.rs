	...
	L: TrieLayout,
{
	...
	fn lookup<'x, 'key>(
		&'x self,
		mut partial: NibbleSlice<'key, L::N>,
				...
				if cp == existing_key.len() && cp == partial.len() {
					...
					*old_val = stored_value;
					...
					if let Some(child) = children[idx].take() {
						...
						if !changed {
							// the new node we composed didn't change. that means our branch is untouched too.
							let n_branch = Node::NibbledBranch(existing_key.to_stored(), children, stored_value);
			...
			Node::Leaf(encoded, stored_value) => {
				...
				if cp == existing_key.len() && cp == partial.len() {
					...
					*old_val = Some(stored_value);
					...
					trace!(
						target: "trie",
						"lesser-common-prefix, not-both-empty (exist={:?}; new={:?}): TRANSMUTE,AUGMENT",
						existing_key.len(),
						partial.len(),
					);
				...
				if NibbleSlice::from_stored(&encoded) == partial {
					...
					*old_val = Some(value);
					...
					trace!(
						...
						partial,
						NibbleSlice::<L::N>::from_stored(&encoded),
					);
...
mod tests {
	...
	fn reference_hashed_null_node() -> <KeccakHasher as Hasher>::Out {
		<ReferenceNodeCodec<BitMap16>
			as NodeCodec<KeccakHasher, <ExtensionLayout as TrieLayout>::N>>::hashed_null_node()
	}
